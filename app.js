...
const SHEETS_URL =
  "https://script.google.com/macros/s/AKfycbxvOTtR79tiTSZMhjz18V3npcxzYZlahHkLB72SaoxbU6QVCd_Z2TIXjK-ynrFW1_F0rQ/exec";
...
function populateFilters() {
  ...
  filterCat.innerHTML =
    ...
    cats
      .map(
        (c) =>
          `<option value="${c}"${c === prevCat ? " selected" : ""}>${c}</option>`,
      )
      .join("");
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    renderAll();
    showToast(
      `✅ Pengeluaran "${expense.name}" berhasil ditambahkan!`,
      "success",
    );
...
syncBtn.addEventListener("click", async () => {
  ...
  if (!navigator.onLine) {
    showToast(
      "📡 Tidak ada koneksi internet. Data akan disinkronkan saat online.",
      "error",
    );
    return;
  }
...
notifBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (permission === "granted") {
      ...
      updateNotifButton();
      showToast(
        "🔔 Notifikasi berhasil diaktifkan! Pengingat jam 20:00 setiap hari.",
        "success",
      );
        ...
        reg.addEventListener("updatefound", () => {
          ...
          newSW.addEventListener("statechange", () => {
            if (
              ...
              showToast(
                "🔄 Update tersedia! Refresh halaman untuk memperbarui.",
                "info",
              );
            }
          });
